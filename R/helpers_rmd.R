...
write_explanation <- function() {
  return(c("**Read distribution plot** - plot shows number of reads assigned during read grouping  ",
           ...
           "**Cutting rates** - plot gives overview of percentage of reads (not recognized as PRIMER DIMERS) that have cut  ",
           "**Frameshift** - plot shows what percentage of reads have frameshift  ",
           "**Unique read share** - shows what is the share of each of the unique reads in total count of all reads  "))
}
...
make_id_rmd <- function(results_folder, cut_buffer = 5) {
  ...
  return(c(write_head("Report breakdown by ID"),
           ...
           "in colour  ",
           "**Mismatches plot** - shows summary of mismatches detected after alignments with distinction for forward",
           "(top plot) and reverse (bottom) reads, mismatches are colored in the same manner as amplicon  ",
           "**Insertions plot** - shows summary of insertions detected after alignments with distinction for forward",
           "(top plot) and reverse (bottom) reads, rach of deletions is shown as right-angled triangle where size of",
           "the insertion coresponds to the right side of the triangle, size and transparency of traingle reflect on",
