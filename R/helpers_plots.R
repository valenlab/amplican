...
amplican_plot_mismatches <- function(alignments, config, id, cut_buffer = 5) {
    ...
    if (dim(idRanges)[1] == 0) {
        return(print("No mismatches to plot."))
    }
    ...
    # FILTER EVENTS forward before primer after amplicon length
    idRanges <- idRanges[!(idRanges$strand == "+" &
                               idRanges$start <= frPrimer[1]), ]
    idRanges <- idRanges[!(idRanges$strand == "+" &
                               idRanges$end >= ampl_len), ]
    # reverse before primer and after amplicon start
    idRanges <- idRanges[!(idRanges$strand == "-" &
                               idRanges$start >= rwPrimer[1]), ]
    idRanges <- idRanges[!(idRanges$strand == "-" &
                               idRanges$start == 1), ]
    if (dim(idRanges)[1] == 0) {
        return(print("No mismatches to plot."))
    ...
    amp_plot <- ggplot2::ggplot(ampl_df,
    ...
    mut_fr <- ggplot2::ggplot() +
        ...
        ggplot2::geom_vline(xintercept = c(start(box), end(box)),
                            linetype = "longdash",
                            colour = "black") +
...
amplican_plot_deletions <- function(alignments, config, id, cut_buffer = 5) {
    ...
    archRanges <- archRanges[!(archRanges$strand == "+" &
                                   archRanges$start <= frPrimer[1]), ]
    archRanges <- archRanges[!(archRanges$strand == "+" &
                                   archRanges$end >= ampl_len), ]
    # reverse before primer and after amplicon start
    archRanges <- archRanges[!(archRanges$strand == "-" &
                                   archRanges$start >= rwPrimer[1]), ]
    archRanges <- archRanges[!(archRanges$strand == "-" &
                                   archRanges$start == 1), ]
    if (dim(archRanges)[1] == 0) {
        return(print("No deletions to plot."))
...
amplican_plot_insertions <- function(alignments, config, id, cut_buffer = 5) {
    ...
    if (dim(idRangesFr)[1] != 0) {
        ...
        idRangesFrX <- as.vector(
            rbind(idRangesFr$start, idRangesFr$start, idRangesFr$end))
...
amplican_plot_cuts <- function(alignments, config, id) {
    ...
    frequency <- cut <- NULL
    arch_plot_top <- ggplot2::ggplot() +
        ggbio::geom_arch(data = archRanges,
                         ggplot2::aes(height = frequency, alpha = frequency,
                                      order = frequency,
                                      size = frequency,
                                      x = start,
                                      xend = end)) +
    ...
    p <- ggbio::tracks(arch_plot_top,
                       amp_plot,
                       arch_plot_bot,
                       heights = c(0.3, 0.03, 0.53),
                       padding = -1,
                       xlim = 1:ampl_len,
                       xlab = "Nucleotide Position Relative to PAM")
    return(p)
