...
getEventInfo <- function(liteString, ID, strand = "*", read_alignment) {
    ...
    if (insCount > 0) {
        ...
        insertionsPairs <- as.numeric(unlist(strsplit(insertions, ",",
                                                      fixed = TRUE)))
        insertionsR <- GRanges(ranges = IRanges(
            start = insertionsPairs[c(TRUE, FALSE)],
    ...
    if (delCount > 0) {
        ...
        deletionsPairs <- as.numeric(unlist(strsplit(deletions, ",",
                                                     fixed = TRUE)))
        deletionsR <- GRanges(ranges = IRanges(
            start = deletionsPairs[c(TRUE, FALSE)],
    ...
    if (mmCount > 0) {
        ...
        mismatches <- unlist(strsplit(mismatches, ",", fixed = TRUE))
        mismatchesR <- GRanges(ranges = IRanges(
            start = as.numeric(mismatches[c(TRUE, FALSE, FALSE)]), width = 1),
    ...
    return(if (length(finalEvents) > 0) finalEvents else GRanges(
...
makeAlignment <- function(configTable, resultsFolder,
    ...
    for (i in 1:dim(configTable)[1]) {
        ...
        if (length(alignmentRanges) > 0) {
            utils::write.csv(as.data.frame(alignmentRanges),
                             paste0(resultsFolder,
                                    "/",
                                    currentID,
                                    "_alignment_ranges.csv"),
                             row.names = FALSE)
        }
    }
    ...
    utils::write.csv(uniqueTable[!uniqueTable$Asigned, ],
                     file = paste(resultsFolder,
                                  "/unassigned_sequences/",
                                  barcode,
                                  "_unassigned_reads.csv",
                                  sep = ""),
