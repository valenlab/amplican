...
############################################
{
  TOTAL_PROCESSORS <- 30       # Set this to the number of processors you want to use.
  ...
  # Test
  #TARGET_FOLDER <- "/Home/ii/rafaelc/Desktop/git/ampliCan/res/Alignment_Toy" # Toy data
  ...
  # Danielle
  TARGET_FOLDER <- "/Home/ii/rafaelc/Desktop/git/ampliCan/src/R/results/run_20150414121918_20141112_F"
...
#   of lines in the .txt file (not counting the header)
{
  ...
  if(length(resultsPath) > 0){
    ...
    configPath      <- list.files(path = resultsPath, pattern=".txt", full.names = TRUE)
...
if(!is.null(allPaths) && !is.null(allFoldersNames) && !is.null(configPath)){
  ...
  foreach(j=1:TOTAL_PROCESSORS , .packages=c()) %dopar% {
    ...
    analyzeConfig(ALL_IN_MODE, ALIGNMENT_DISTANCE, SAME_CUTS, PRIMERDIMER, DIMERBASE,
                  BOTHDIMERS, processorsSubDataframes[[j]], analysisPath, j, resultsPath)
    
    
  }
  
  # Put all the logs toguether
  totalLogs <- unifyFiles(analysisPath, "SUBLOG", "myFinalLog.txt",FALSE,TRUE,FALSE)
  
  # Now, let put all the configs toguether
