...
class Alignment_Result{
	...
	public:
		...
		string to_string(){
			...
			if(this==NULL){
				...
				for(int i=0; i<totalChunks; i++){
					...
					toReturn = toReturn +
					patternPiece + "    " + std::to_string(80*(i+1) - 1) + "\n"+
					markupPiece + "    \n"+
					subjectPiece + "    \n\n";
				}
		...
		*/
		string to_ggplot2_string(){ return "";}
...
*/
inline int maxRow(int** matrix, const int &rowIndex, const int &totalColumns,
				  int &maximumIndex, const int &offset = 0){
    ...
    for(int i=offset+1; i<totalColumns; i++){
...
*/
inline int maxColumn(int** matrix, const int &columnIndex, const int &totalRows,
				  int &maximumIndex, const int &offset = 0){
	...
	//For each of the elements in the row
    for(int i=offset+1; i<totalRows; i++){
...
int align(int** grid, int** vertical, int** horizontal, char** tracebackGrid,
	...
	int newGap = 0;
	int extendGap = 0;
	//int otherMatrixGap = 0; //Experimental
	...
	*/
	if(gapEnding == false){
		...
		//Which sequence is bigger?
		//The Pattern (adjust j) 
		if(lengthPattern>lengthSubject){

			cout<<"B"<<endl;

			//Search the maximum in the last row
			maxScore = maxRow(grid,i,j,maximumIndex,1);

			//Adjust the alignments and the traceback start
			while(j!=maximumIndex){
		...
		//The subject (adjust i)
		else if(lengthPattern<lengthSubject){

			cout<<"C"<<endl;

			//Search the maximum in the last column
			maxScore = maxColumn(grid,j,i,maximumIndex,1);
	...
	while(i>0 || j>0){
		...
		switch(tracebackGrid[i][j]){
			...
			case '\\':
				patternResult += sequencePattern[j-1]; 
                subjectResult += sequenceSubject[i-1];
				if(sequencePattern[j-1] == sequenceSubject[i-1]){
					markup += '|';
				}
				else{
					markup += '.';
			...
			case 'V':
				...
				*/
				...
				//If we come back, then we have a deletion
				if(direction=='G' && i!=0){
					Indel_Info newDeletion(i,indelEnd,true); 
			...
			case 'H':
				...
				*/
				...
				//If we come back, then we have an insertion
				if(direction=='G' && j!=0){
					Indel_Info newInsertion(j,indelEnd,true); 
	...
	myResult = new Alignment_Result(sequencePattern, sequenceSubject, matrix, gapOpening,
	                       gapExtension, gapEnding, patternResult.length(),
	                       maxScore, patternResult, markup, subjectResult,
	                       insertions.size(), deletions.size(),0,
...
*/
int nw(const string &sequencePattern,const string &sequenceSubject,
			  const string &matrix, const int &gapOpening,
			  const int &gapExtension, const bool &gapEnding,
			  Alignment_Result* &myResult){
	...
	align(grid, vertical, horizontal, tracebackGrid, tracebackVertical,
		  ...
		  gapOpening, gapExtension, gapEnding, myResult);
...
string nwRCPP(string sequencePattern, string sequenceSubject,
	...
	status = nw(sequencePattern, sequenceSubject, matrix, gapOpening,
				gapExtension, gapEnding, myResult);
