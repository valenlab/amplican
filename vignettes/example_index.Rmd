...
reads_indel <- c(sum(configDF$Reads_Indel[!configDF$Control]), # Treatment
               total_reads_tmt - sum(configDF$Reads_Indel[!configDF$Control]),
               sum(configDF$Reads_Indel[configDF$Control]), # Control
               total_reads_ctr - sum(configDF$Reads_Indel[configDF$Control]))
reads_indel_per <- round(reads_indel*100/total_reads)
reads_indel_per[is.na(reads_indel_per)] <- 0
names(reads_indel_per) <- c(
  paste0('DSB in Treatment\n', reads_indel[1], ' (', reads_indel_per[1], '%)'),
  paste0('No DSB in Treatment\n', reads_indel[2], ' (', reads_indel_per[2], '%)'),
  paste0('DSB in Control\n', reads_indel[3], ' (', reads_indel_per[3], '%)'), 
  paste0('No DSB in Control\n', reads_indel[4], ' (', reads_indel_per[4], '%)'))
waffle(reads_indel_per, legend_pos = 'bottom', 
       title = "Reads with mutation", 
