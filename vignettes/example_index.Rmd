...
**Assigned Reads/Unassigned Reads** - how many reads have been assigned/not assigned to any of the
           experiments  
...
library(ggplot2)
library(waffle)
summaryDF <- read.csv(params$barcode_summary, stringsAsFactors = FALSE)
...
# PRIMER DIMER reads
configDF <- read.csv(params$config_summary, stringsAsFactors = FALSE)
height <- amplican:::plot_height(length(unique(configDF$Barcode)))

PD_reads <- c(sum(configDF$Reads_noPD), sum(configDF$PRIMER_DIMER))
PD_reads_per <- round(PD_reads*100/sum(configDF$Reads))
PD_reads_per[is.na(PD_reads_per)] <- 0
names(PD_reads_per) <- c(paste0('Good Reads\n', PD_reads[1], ' (', PD_reads_per[1], '%)'), 
                         paste0('PRIMER DIMER Reads\n', PD_reads[2], ' (', PD_reads_per[2], '%)'))
waffle(PD_reads_per, legend_pos = 'bottom', 
       title = "\n\nReads marked as PRIMER DIMERS", 
       rows = 10, colors = c('#E69F00', '#000000'))
```
...
```{r echo = F}
total_reads <- sum(configDF$Reads_noPD)
total_reads_ctr <- sum(configDF$Reads_noPD[configDF$Control])
total_reads_tmt <- sum(configDF$Reads_noPD[!configDF$Control])
reads_cut <- c(sum(configDF$Reads_Cut[!configDF$Control]), # Treatment
               total_reads_tmt - sum(configDF$Reads_Cut[!configDF$Control]),
               sum(configDF$Reads_Cut[configDF$Control]), # Control
               total_reads_ctr - sum(configDF$Reads_Cut[configDF$Control]))
reads_cut_per <- round(reads_cut*100/total_reads)
reads_cut_per[is.na(reads_cut_per)] <- 0
names(reads_cut_per) <- c(
  paste0('DSB in Treatment\n', reads_cut[1], ' (', reads_cut_per[1], '%)'),
  paste0('No DSB in Treatment\n', reads_cut[2], ' (', reads_cut_per[2], '%)'),
  paste0('DSB in Control\n', reads_cut[3], ' (', reads_cut_per[3], '%)'), 
  paste0('No DSB in Control\n', reads_cut[4], ' (', reads_cut_per[4], '%)'))
waffle(reads_cut_per, legend_pos = 'bottom', 
       title = "Reads with Double Stranded Break (Cut rate)", 
...
frameshift <- c(sum(configDF$Reads_Frameshifted_Overlapping[!configDF$Control]), # Treatment
                total_reads_tmt - sum(configDF$Reads_Frameshifted_Overlapping[!configDF$Control]),
                sum(configDF$Reads_Frameshifted_Overlapping[configDF$Control]), # Control
                total_reads_ctr - sum(configDF$Reads_Frameshifted_Overlapping[configDF$Control]))
frameshift_per <- round(frameshift*100/total_reads)
frameshift_per[is.na(frameshift_per)] <- 0
names(frameshift_per) <- c(
  paste0('Frameshift in Treatment\n', frameshift[1], ' (', frameshift_per[1], '%)'),
...
```{r fig.width=8, fig.height=height, echo = F}
library(reshape2)
library(ggthemes)
summaryDF_per <- summaryDF
filters <- c('bad_base_quality', 'bad_average_quality', 'bad_alphabet')
summaryDF_per[, filters] <- round(summaryDF_per[, filters]*100/rowSums(summaryDF_per[, filters]))
...
summaryDF_per[, c('filtered_read_count', 'bad_read_count')] <- 
  round(summaryDF_per[, c('filtered_read_count', 'bad_read_count')]*100/summaryDF_per$read_count)
summaryDF_per[is.na(summaryDF_per)] <- 0

quality_melt <- melt(summaryDF_per, id.vars = c('Barcode'), 
                     measure.vars = c('filtered_read_count', 'bad_read_count'))
quality_det_melt <- melt(summaryDF_per,
                     id.vars = c('Barcode'),
                     measure.vars = c('bad_base_quality',
                                      'bad_average_quality',
                                      'bad_alphabet'))
assignment_melt <- melt(summaryDF_per,
                        id.vars = c('Barcode'),
