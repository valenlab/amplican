...
During `amplicanPipeline` these columns are added to the config file:
 ...
 - **Reads_Cut** - number of deletions overlapping with user specified UPPER CASE group in the amplicon (extended by the buffer), cuts have to be deletions confirmed with the reverse strand when using paired-end reads, for more information check 
`amplicanConsensus`  
...
"events_filtered_shifted_normalized.csv" is save, probably it is the file you should use
...
```{r, echo=TRUE}
aln <- readRDS(
  system.file("extdata", "results", "alignments", "AlignmentsExperimentSet.rds", 
              package = "amplican"))
