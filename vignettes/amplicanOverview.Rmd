...
to which corespond 17 and 29 reads. Two reads are rejected for barcode_1 due to bad alphabet. Each of the barcodes has
unique reads, which means forward and reverse reads are compacted when they are identical. There is 11 unique reads for 
barcode_1 and 20 for barcode_2. We can succesfully asign all the reads for each barcode as this is artificial dataset.
Normally you will probably see only half of your reads being assigned to the barcodes. Reads are assigned when for 
forward read we can find forward primer and for reverse read we can find reverse primer. Primers have to be perfectly
matched. Nevertheless, there is option `fastqreads = 0.5` which changes method of assigning reads to each IDs. With 
this option specified only one of the reads (forward or reverse) have to have primer perfectly matched.
...
`amplicanAnalysis` as "2", which is default, you will find here two files, one with just alignment forward read, 
...
In `unassigned_sequences` folder you can find detailed information about unassigned reads. In example dataset there
is no unassigned reads.
...
version of the report. If you have runned our example analysis, then you can open one of the reports with Rstudio and 
try kniting it now! Otherwise you should change the path at the top of this file to match your path to the package, as
you have pregenerated path (running `amplicanPipeline` does that for you). 

```{r, echo=TRUE}
system.file("extdata", "results/reports", package = "amplican") #folder with example reports

# you can run for example:
# rmarkdown::render(system.file("extdata", "results/reports/report_id.Rmd", package = "amplican")) #might be time consuming
```
...
implemented in C for speed. When you want to have more control over alignments and you need more advanced options use `amplicanAnalysis`. This function has many parameters which can be used to bend our pipeline to your needs. 
Read more about `amplicanAnalysis` on the help page `?amplican::amplicanAnalysis`.

### Making reports

Reports are made for user convienience, they are powerfull and they:
