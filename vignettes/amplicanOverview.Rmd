...
**guideRNA** - put your guideRNA here, if he has to be reverse complemented put "1" into **Direction** field, 
we will use this field to make sure your guideRNA is inside the amplicon  
...
`amplican_default` function.

```{r amplican_default, echo=TRUE}
# library(amplican) #don't load for time
# example MiSeq dataset files
config <- system.file("extdata", "config.txt", package = "amplican")
...
# actual amplican run
# amplican_default(config, fastq_folder, results_folder) #might be time consuming
...
something goes wrong you should examin file and follow instructions inside.
...
```{r, echo=FALSE}
barcodeFilters <- read.csv(system.file("extdata/results", "barcode_reads_filters.csv", 
                                       package = "amplican"), sep = "\t", header = T)
...
```{r, echo=FALSE}
config_summary <- read.csv(system.file("extdata/results", "config_summary.csv", 
                                       package = "amplican"), sep = "\t", header = T)
...
Contains alignments, for each ID there is separate folder. If you specified `write_alignments` as "2" you will
find here two files, one with just alignment forward read, reverse read and count. Other file will contain very
