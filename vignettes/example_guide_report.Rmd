...
config$indel_percentage <- config$Reads_Indel * 100/config$Reads_noPD
config$indel_percentage[is.nan(config$indel_percentage)] <- 0  

ggplot(data = config, aes(x = as.factor(guideRNA), y = indel_percentage, order = guideRNA, fill = guideRNA)) +
  geom_boxplot() +
  xlab('guideRNA') + 
