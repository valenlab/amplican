...
ampliconTable <- aggregate(cbind(Reads, PRIMER_DIMER, 
                                 Reads_Cut, Reads_Frameshifted) ~ group, data = config, sum)
...
src = sapply(seq_along(uniqueAmlicons), function(i) {
  ...
  knitr::knit_expand(text = c(
    ...
    paste('```{r cuts-{{i}}, echo = F, results = "asis", ',
          'fig.width=25, message=F, warning=F}', collapse = ''), 
    paste('amplican::plot_cuts(alignments_cons, config, {{ID}},',
          ' params$cut_buffer, params$xlab_spacing)', collapse = ''), 
    '```\n',
    "### Variants", 
    paste('```{r var-{{i}}, echo = F, results = "asis", ',
          'message=F, warning=F}', collapse = ''),
    paste('amplican::plot_variants(alignments_cons, config, {{ID}}, ',
