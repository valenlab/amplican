...
ampliconTable <- aggregate(cbind(Reads, PRIMER_DIMER, 
                                 Reads_Indel, Reads_Frameshifted) ~ group, data = config, sum)
...
ampliconTable$indel_percentage <- ampliconTable$Reads_Indel * 100/ampliconTable$Reads_noPD
ampliconTable$indel_percentage[is.nan(ampliconTable$indel_percentage)] <- 0  

ggplot(data = ampliconTable, aes(x = as.factor(group), y = indel_percentage, order = group)) +
  geom_bar(stat = 'identity') +
  ...
  ylim(0,100) +
  coord_flip() +
  geom_text(aes(x = as.factor(group), y = indel_percentage, label = Reads_Indel), hjust = -1)
```  
