...
src = sapply(config$ID, function(i) {
  amplicon <- config$Amplicon[config$ID == i][1]
  plot_bp <- nchar(gsub("[a-z]", "", amplicon)) + params$cut_buffer * 2
  fig_width <- max(round(plot_bp * 25 / 150), 10)
  knitr::knit_expand(text = c(
    ...
    paste('```{r var-{{i}}, echo = F, message=F, results = "asis", ',
          'fig.width={{fig_width}}, message=F, warning=F}', collapse = ''),
    paste('p <- amplican::plot_variants(alignments, config, "{{i}}", ',
