...
params:
  alignments: !r system.file('extdata', 'results', 'alignments_events.csv', package = 'amplican')
  config_summary: !r system.file('extdata', 'results', 'config_summary.csv', package = 'amplican')
  unassigned_folder: !r system.file('extdata', 'results',  'alignments', 'unassigned_sequences', package = 'amplican')
  top: 5
vignette: >
...
```{r plot total reads, echo=FALSE, fig.height=height, fig.width=14, message=F, warning=FALSE}
...
```{r plot PRIMER DIMER percentage, echo=FALSE, fig.height=height, fig.width=14, message=F, warning=FALSE}
...
```{r plot frame shift percentage, echo=FALSE, fig.height=height, fig.width=14, message=F, warning=FALSE}
...
if (!is.na(config$Forward_Reads_File) && !is.na(config$Reverse_Reads_File)) {
  ub <- list.files(params$unassigned_folder)
  pure_bname <- gsub("_unassigned_reads.csv", "", ub)
  
  cat("# Top unassigned reads  \n\n***  \n\n")
  for (i in seq_along(ub)) {
    cat(paste0("## ", pure_bname[i], "  \n\n"))
    unassigned_reads <- read.csv(file.path(params$unassigned_folder, ub[i]), stringsAsFactors = FALSE)
    ...
    if (dim(unassigned_reads)[1] == 0) {
      ...
      topN <- if (dim(unassigned_reads)[1] < params$top) dim(unassigned_reads)[1] else top
      cat('\n')
      ...
      cat("\n")
      cat(paste("<PRE>", paste(amplican_print_reads(unassigned_reads[1:topN, 'Forward'],
                                                    unassigned_reads[1:topN, 'Reverse']),
                               collapse = "\n"), "</PRE>"))
      cat("\n\n")
    }
  }
