...
```{r plot_filter_per, echo=FALSE, fig.height=height, fig.width=14, message=F, warning=FALSE}
barcodeTable <- aggregate(cbind(Reads, Reads_Filtered, PRIMER_DIMER, 
                                Low_Score, Reads_Indel, Reads_Frameshifted) ~ Barcode, 
                          data = config, sum)
...
barcodeTable$indel_percentage <- 
  barcodeTable$Reads_Indel * 100/barcodeTable$Reads_Filtered
...
ggplot(data = barcodeTable, aes(x = as.factor(Barcode), y = indel_percentage, order = Barcode)) +
  ...
  geom_text(aes(x = as.factor(Barcode), 
                y = indel_percentage, label = Reads_Indel), hjust = -1)
``` 
