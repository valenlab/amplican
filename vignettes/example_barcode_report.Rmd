...
barcodeTable <- aggregate(cbind(Reads, PRIMER_DIMER, Reads_Indel, Reads_Frameshifted) ~ Barcode, data = config, sum)
...
barcodeTable$indel_percentage <- barcodeTable$Reads_Indel * 100/barcodeTable$Reads_noPD
barcodeTable$indel_percentage[is.nan(barcodeTable$indel_percentage)] <- 0  

ggplot(data = barcodeTable, aes(x = as.factor(Barcode), y = indel_percentage, order = Barcode)) +
  geom_bar(stat = 'identity') +
  ylab('Percentage of reads (not marked as PRIMER DIMERS) that have indel')  +
  theme(axis.text = element_text(size=12),
        axis.title = element_text(size=14, face = 'bold')) +
  ylim(0,100) +
  xlab('Barcode') +
  coord_flip() +
  geom_text(aes(x = as.factor(Barcode), y = indel_percentage, label = Reads_Indel), hjust = -1)
``` 
...
if (!is.na(config$Forward_Reads_File) && !is.na(config$Reverse_Reads_File) &&
    file.exists(params$unassigned_folder)) {
